...
- **免安装（portable）版**：数据放在安装目录（`<免安装根目录>\data\hermes-home\state.db`）。
  它没有注册表项也没有快捷方式线索，所以本工具会**从正在运行的 Hermes 进程路径反推**该目录——
  只要 Hermes 在运行就零配置自动识别，无需填任何东西；
  若希望 Hermes 没开时也持续检测，可在**设置页 → Hermes Agent 数据目录**固化路径
  （填根目录、`…\data\hermes-home` 或 `state.db` 文件路径都行），保存后约 3 秒生效、无需重启；留空恢复自动探测

自动探测顺序：设置页指定 → `HERMES_HOME` → 平台默认 home / `~/.hermes` → 运行中进程反推的免安装目录（后两者会合并，可同时统计多个安装）。
...
- 免安装（portable）版 Hermes 只能从**运行中的进程**反推数据目录，因此 Hermes 未运行时该通道暂时无数据（历史数据不会丢，Hermes 下次启动后按原时间补录）；要避免这种情况，可在设置页固化路径。已在本机实测 `Hermes Agent CN Desktop Portable`（schema v22，`data\hermes-home\state.db`）自动识别并正确入账
...
- ✨ **免安装（portable）版零配置识别**：从正在运行的 Hermes 进程路径反推 `…\data\hermes-home`（30 秒一次的轻量进程枚举，不引入额外依赖；无需管理员权限）；设置页同时新增「Hermes Agent 数据目录」，可固化路径（根目录 / `data\hermes-home` / `state.db` 三种写法都接受），保存后约 3 秒生效、无需重启。已在本机 `Hermes Agent CN Desktop Portable`（schema v22）上实测：自动识别 + 口径正确 + 重复扫描不重复计数
